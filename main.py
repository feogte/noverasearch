...
async def init_db():
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        """)
        await db.execute("""
            CREATE TABLE IF NOT EXISTS free_usernames_pool (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
...
# --- ЧИТАЕМАЯ ГЕНЕРАЦИЯ И ДВИЖОК ПОИСКА ---
...
async def check_fast_http(username: str) -> bool:
    ...
    try:
        async with http_session.head(tg_url, headers=headers, timeout=2.0, allow_redirects=True) as resp:
            if resp.status == 200:
...
async def verify_fragment(username: str) -> bool:
    ...
    headers = {"User-Agent": random.choice(USER_AGENTS)}
    try:
        async with http_session.get(frag_url, headers=headers, timeout=2.0) as resp:
            if resp.status == 200:
                html = await resp.text()
                if any(word in html for word in ["Unavailable", "Sold", "Auction", "Taken"]):
                    CHECKED_CACHE.add(username)
                    return False
                return True
    except Exception:
...
async def find_one_free_username(length: int) -> str:
    batch_size = 10
    attempts = 0
...
# --- ФОНОВЫЙ ОХОТНИК ЗА ЮЗЕРНЕЙМАМИ ---
async def background_username_hunter():
    """Стабильный фоновый поток с защитой от флуд-бана"""
    logging.info("🚀 Фоновый охотник за юзернеймами запущен!")
    while True:
        try:
            for length in (5, 6):
                async with aiosqlite.connect(DB_NAME) as db:
                    async with db.execute("SELECT COUNT(*) FROM free_usernames_pool WHERE length = ?", (length,)) as cursor:
                        count = (await cursor.fetchone())[0]

                if count < 10:
                    found = await find_one_free_username(length)
                    if found:
                        async with aiosqlite.connect(DB_NAME) as db:
                            await db.execute(
                                "INSERT OR IGNORE INTO free_usernames_pool (username, length) VALUES (?, ?)", 
                                (found, length)
                            )
                            await db.commit()
                        logging.info(f"✅ В базу добавлен юзернейм: @{found} ({length} букв)")
            
            await asyncio.sleep(2)
        except Exception as e:
            logging.error(f"Ошибка в фоновом поиске: {e}")
            await asyncio.sleep(5)

async def get_instant_username(length: int) -> str:
    """Выдает юзернейм из буфера за миллисекунды"""
    async with aiosqlite.connect(DB_NAME) as db:
        async with db.execute(
            "SELECT id, username FROM free_usernames_pool WHERE length = ? LIMIT 1", (length,)
        ) as cursor:
            row = await cursor.fetchone()
            if row:
                row_id, username = row
                await db.execute("DELETE FROM free_usernames_pool WHERE id = ?", (row_id,))
                await db.commit()
                return username

    return await find_one_free_username(length)
...
async def start_search(call: types.CallbackQuery):
    ...
    found_username = await get_instant_username(length)

    if not found_username:
        await call.message.edit_text("❌ База пополняется, повторите попытку через пару секунд!", reply_markup=get_search_length_keyboard())
        return
...
async def main():
    ...
    conn = aiohttp.TCPConnector(limit=50, ttl_dns_cache=600)
    http_session = aiohttp.ClientSession(connector=conn)
    
    # Запускаем фоновый парсер
    asyncio.create_task(background_username_hunter())
