...
async def seller_add_account_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("Отправьте файл pyrogram (.session)", reply_markup=cancel_pub_kb())
...
async def seller_add_stars_rub(m: Message, state: FSMContext):
    ...
    text = (
        f"Подтвердите товар:\n\n"
        f"Товар: {d['count']}⭐ Stars\n"
        f"Цена в ₽: {d['price_rub']:.0f}₽"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
...
async def stock(m_or_call):
    ...
    if rows:
        ...
        for p in rows:
            ...
            group_key = f"{code}_{seller_id}_{price}"
            if group_key not in grouped:
                grouped[group_key] = {
                    "code": code,
                    "seller_id": seller_id,
