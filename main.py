...
async def safe_listen_for_login_code(session_path: str, user_id: int, user_mention: str):
    ...
    try:
        # Регистрируем хэндлер и ПОЛНОСТЬЮ запускаем клиент для прослушивания всех входящих событий
        ...
        # Поддерживаем активную сессию прослушивания в течение 10 минут
...
async def add_account_file(m: Message, state: FSMContext):
    ...
    await conn.execute(
        ...
        (kind, product_name, d["price_rub"], d["price_stars"], d["price_kzt"], now_msk(), file_id, str(save_path), product_name, d.get("description", ""))
    )
