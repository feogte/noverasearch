...
# --- СКОРОСТНОЙ ДВИЖОК ПОИСКА (ТОЛЬКО БУКВЫ) ---
def generate_candidate(length: int) -> str:
    """Генерация кандидатов строго из латинских букв"""
...
async def check_fast_http(username: str) -> bool:
    ...
    try:
        # HEAD-запрос проверяет статус за миллисекунды без скачивания тела страницы
...
async def verify_fragment(username: str) -> bool:
    """Точная проверка через Fragment для подтверждения свободы юзернейма"""
    frag_url = f"https://fragment.com/username/{username}"
    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    try:
        async with http_session.get(frag_url, headers=headers, timeout=1.0) as resp:
            if resp.status == 200:
...
async def find_free_username_ultra_fast(length: int) -> str:
    # Параллельно проверяем пачками по 40 юзернеймов
    batch_size = 40
    max_attempts = 15
    ...
    for _ in range(max_attempts):
        ...
        for idx, is_free in enumerate(results):
            if is_free is True:
                ...
                # Перепроверяем на Fragment перед выдачей пользователю
                if await verify_fragment(candidate):
                    return candidate
...
async def start_search(call: types.CallbackQuery):
    ...
    await call.message.edit_text("⚡️ Ищем свободный юзернейм из букв...")
    
    found_username = await find_free_username_ultra_fast(length)
    
    if not found_username:
        await call.message.edit_text("❌ Не удалось найти свободный буквенный вариант за один проход. Нажмите поиск еще раз!", reply_markup=get_search_length_keyboard())
        return
