...
from telethon.errors import UsernameInvalidError, UsernameOccupiedError, SessionPasswordNeededError, FloodWaitError
...
# Глобальные переменные для ускорения
http_session: aiohttp.ClientSession = None
CHECKED_CACHE = set()  # Кэш проверенных занятых юзернеймов
...
# --- СУПЕР-БЫСТРЫЙ ИСПОЛНИТЕЛЬ ПРОВЕРКИ ---
def generate_candidate(length: int) -> str:
    return ''.join(random.choices(string.ascii_lowercase, k=length))

async def check_fragment_fast(username: str) -> bool:
    url = f"https://fragment.com/username/{username}"
    try:
        async with http_session.get(url, timeout=1.5) as resp:
            if resp.status == 200:
...
async def check_telegram_fast(username: str) -> bool:
    try:
...
async def verify_single_candidate(username: str):
    if username in CHECKED_CACHE:
        return None
        
    is_tg_free = await check_telegram_fast(username)
    if not is_tg_free:
        CHECKED_CACHE.add(username)
        return None
        
    is_fragment_free = await check_fragment_fast(username)
    if not is_fragment_free:
        CHECKED_CACHE.add(username)
        return None
        
    return username

async def find_free_username_fast(length: int) -> str:
    # Запускаем пакетную генерацию по 10 за раз
    batch_size = 10
    max_batches = 15
    
    for _ in range(max_batches):
        candidates = [generate_candidate(length) for _ in range(batch_size)]
        tasks = [verify_single_candidate(cand) for cand in candidates]
        ...
        await asyncio.sleep(0.05)  # Микро-пауза для снижения нагрузок
...
async def start_search(call: types.CallbackQuery):
    ...
    found_username = await find_free_username_fast(length)
...
async def main():
    ...
    # Инициализация единой клиентской сессии aiohttp
