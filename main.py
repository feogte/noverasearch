...
async def check_fast_http(username: str) -> bool:
    ...
    try:
        async with http_session.head(tg_url, headers=headers, timeout=0.6, allow_redirects=True) as resp:
            if resp.status == 200:
...
async def find_one_free_username(length: int) -> str:
    ...
    for _ in range(10):
        ...
        for idx, is_free in enumerate(results):
            if is_free is True:
                cand = candidates[idx]
...
async def background_username_hunter():
    """Фоновый поток: накапливает буфер свободных юзернеймов в базе 24/7"""
    while True:
        try:
            for length in (5, 6):
                async with aiosqlite.connect(DB_NAME) as db:
