...
async def products(m: Message, kind: str):
    ...
    if kind == "account":
        ...
        for p in rows:
            ...
            # Группировка строго по названию/коду и ID продавца (разные продавцы будут иметь разные группы без упоминания юзов)
        ...
        for item in grouped.values():
            ...
            btn_text = f"{flag}{item['title']} [{item['price']:.0f}₽]{cnt_str}"
...
async def stock(m_or_call):
    ...
    if rows:
        ...
        for p in rows:
            ...
            group_key = f"{display_title}_{seller_id}"
            if group_key not in grouped:
                grouped[group_key] = {
                    "title": display_title,
                    "seller_id": seller_id,
        ...
        for item in grouped.values():
            flag = get_flag(item["title"])
            cnt_str = f" ({item['count']} шт)" if item["count"] > 1 else ""
            
            # Если выложил главный админ (ID равен ADMIN_ID или 0) — юзернейм продавца НЕ пишется
            ...
            btn_label = f"🗑 {flag}{item['title']}{cnt_str}{seller_str} [{item['price']:.0f}₽]"
