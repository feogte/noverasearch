...
async def products(m: Message, kind: str):
    ...
    if kind == "account":
        ...
        for p in rows:
            ...
            # Уникальная группировка без вывода юзернейма продавца для покупателя
...
async def stock(m_or_call):
    ...
    if rows:
        for p in rows:
            seller_str = f" - @{p['seller_username']}" if p.get('seller_username') else (f" - ID {p['seller_id']}" if p['seller_id'] != 0 else " - Администратор")
            display_title = p["name"] if p["name"] else extract_code_prefix(p["phone"])
            flag = get_flag(display_title)
            
            # Строка вида: +1 / +1 5 лет - @username продавца
