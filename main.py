...
from database import init_db, get_or_create_user, increment_search_count, save_username_to_buffer
from searcher import telethon_client, find_free_username, PHONE_NUMBER
...
async def process_auth_code(message: types.Message, state: FSMContext):
    ...
    # Очистка кода: удаляем точки, пробелы, дефисы и другие символы
...
async def admin_grant(call: types.CallbackQuery):
    ...
    import aiosqlite
    async with aiosqlite.connect("novera_search.db") as db:
...
async def main():
    ...
    # 1. Автоматическое создание файла базы данных и таблиц при старте на хосте
    await init_db()
    logging.info("База данных инициализирована.")
    
    # 2. Подключаем клиент Telethon без блокировки консоли
    await telethon_client.connect()
