...
async def products(m: Message, kind: str):
    ...
    if kind == "account":
        ...
        for p in rows:
            ...
            # Разделение логики: если указано специальное наименование (например, "+1 5 лет"),
            # группируем по точному имени; иначе — по сочетанию кода и продавца.
            if name_val.strip() != "" and not name_val.startswith("+") and len(name_val.split()) > 1:
                group_key = f"name_{name_val}"
                display_title = name_val
            else:
                seller = p["seller_id"]
                group_key = f"seller_{code}_{seller}"
                display_title = code
