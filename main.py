...
async def init_db():
    ...
    """)
    # Миграция колонок для старых БД
...
async def adm_seller_reject(call: CallbackQuery):
    ...
    if req:
        ...
        await conn.commit()
    await conn.close()
    await call.message.edit_text("❌ Заявка на продавца отклонена.")

# --- Выкладка товаров продавцом ---
...
async def seller_add_account_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("отправьте файл pyrogram")
...
async def seller_add_stars_start(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.answer("введите количество звезд")
...
async def seller_add_stars_rub(m: Message, state: FSMContext):
    try: price = float(m.text.replace(",", "."))
    except ValueError: return await m.answer("Введите цену в рублях:")
    await state.update_data(price_rub=price)
...
async def approve_purchase(call: CallbackQuery):
    ...
    # Если товар стороннего продавца — уведомляем его о продаже и даем ссылку на выплату
...
async def manage_stock_item(call: CallbackQuery):
    ...
    conn = await db()
    prods = await all_rows(conn, "SELECT * FROM products WHERE stock>0 AND (phone LIKE ? OR name LIKE ? OR phone LIKE ?)", (f"{key}%", f"{key}%", f"{raw_digits}%"))
    await conn.close()
