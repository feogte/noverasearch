...
# HTTP-сессия и локальный кэш занятых имен
...
# --- СВЕРХБЫСТРЫЙ ДВИЖОК ПРОВЕРКИ ---
def generate_candidate(length: int) -> str:
    # Исключаем редкие сочетания для ускорения поиска
    vowels = "aeiou"
    consonants = "bcdfghjklmnpqrstvwxyz"
    res = []
    for i in range(length):
        res.append(random.choice(vowels) if i % 2 == 1 else random.choice(consonants))
    return "".join(res)

async def check_fast_http(username: str) -> bool:
    """Проверка доступности через t.me и Fragment с жестким таймаутом"""
    if username in CHECKED_CACHE:
        return False

    headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}
    
    # 1. Быстрая проверка Web Telegram
    try:
        tg_url = f"https://t.me/{username}"
        async with http_session.get(tg_url, headers=headers, timeout=0.5) as resp:
            text = await resp.text()
            # Если в теле страницы есть класс tgme_page_extra, аккаунт/канал существует
            if "tgme_page_extra" in text or "If you have Telegram, you can contact" in text:
                CHECKED_CACHE.add(username)
                return False
    except Exception:
        pass

    # 2. Быстрая проверка Fragment
    try:
        frag_url = f"https://fragment.com/username/{username}"
        async with http_session.get(frag_url, headers=headers, timeout=0.5) as resp:
            if resp.status == 200:
...
async def find_free_username_ultra_fast(length: int) -> str:
    # Параллельно проверяем пачками по 20 штук
    batch_size = 20
    max_attempts = 10
    
    for _ in range(max_attempts):
        candidates = [generate_candidate(length) for _ in range(batch_size)]
...
async def start_search(call: types.CallbackQuery):
    ...
    found_username = await find_free_username_ultra_fast(length)
    
    if not found_username:
        await call.message.edit_text("❌ Не удалось мгновенно найти вариант. Попробуйте еще раз!", reply_markup=get_search_length_keyboard())
        return
...
async def main():
    ...
    # Единый HTTP пул для предельной скорости
